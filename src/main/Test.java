...
public class Test {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < substrateFile.size(); i++) { // run substrate
			...
			List<Vertex> bellmanFordPath  = sObj.defineShortestPathBellmanFord(sourceNode, destinationNode, 5);
			

			int totalDistance = sObj.getTotalDistance(); // distance from source to destination
			int BellmanTotalDistance = sObj.getTotalDistance();
			...
			System.out.println("=============================================");
			System.out.printf("Bellman-Ford Algorithm - Path %d to %d:\t%s\n", sourceNode, destinationNode, Arrays.toString(bellmanFordPath.toArray()));
			System.out.printf("Total distance:\t%d\n", BellmanTotalDistance);
